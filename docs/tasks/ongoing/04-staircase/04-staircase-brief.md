...
- **Stream rooms in and out** around the player — load the neighborhood you're entering, unload what's far behind, so the world is effectively infinite without rendering infinitely. This includes streaming the _adjacent floor_ while you're mid-climb, since the staircase is walkable.
...
- Enforce the **deterministic `up`/`down` rule**: climbing from a vestibule lands you at the _same_ vestibule one floor up, so vertical loops close exactly (climb, walk, descend, walk back = home). Same for horizontal moves.
...
- **Loops close** — this is the ℤ² lattice property from Unit 02; this unit must make it true _in the walkable world_, not just in the algebra.
...
- Unit 03 spec — the room module, the vestibule + staircase _geometry_ placeholder, WASD + mouselook locomotion, the fog that hides the shaft, the audio bus.
...
- **Continuous vertical streaming**: mid-climb, when does the destination floor's room become real? The staircase being walkable means there's a moment you can see _into_ the next floor — how much of it must render, and at what fidelity, before you arrive?
